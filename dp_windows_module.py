...
Columns:
S.No, Server Name, Environment, Status, OS Version, OS Edition, Security Patch,
Memory Allocated (GB), CPU Count, Memory Usage %, CPU Load Avg %,
C, D, E, F, G, H, I, T, S, Last Checked, Check Status, Error
"""
...
def logical_columns() -> List[str]:
    cols = ["S.No", "Server Name", "Environment", "Status", "OS Version", "OS Edition", "Security Patch", "Memory Allocated (GB)", "CPU Count", "Memory Usage %", "CPU Load Avg %"]
    cols += LETTERS
...
def collect_win_metrics(cfg: Dict[str, Any], host: WinHost) -> WinHealth:
    ...
    try:
        ...
        ver_year = m.group(1) if m else caption
        h.os_version = ver_year
        sku = str(data.get("Edition", "-"))
        sku_map = {"8": "Datacenter", "7": "Standard", "12": "Datacenter Core", "13": "Standard Core"}
        h.os_edition = sku_map.get(sku, "Unknown")
...
class WindowsMonitorApp(ttk.Frame):
    ...
    def _pick_ps(self):
        ...
        if p:
            self.ps_path_var.set(p)
            self.cfg["powershell_path"] = p
            save_config(self.cfg)
    ...
    def _apply_persisted_row(self, name: str, h: Dict[str, Any]):
        ...
        vals[colidx["CPU Load Avg %"]] = self._mark_cell(None if cpu_pct is None else (cpu_pct < 80.0), f"{cpu_pct:.1f}%" if cpu_pct is not None else "-")
        for L in LETTERS:
    ...
    def _apply_result(self, name: str, host: WinHost, h: WinHealth):
        ...
        self.last_health[name] = {
            ...
            "os_edition": h.os_edition,
            "security_patch": h.security_patch,
            "mem_gb": h.mem_gb,
    ...
    def _add_dialog(self):
        WinHostEditor(self, on_save=self._add_host)
...
class WinHostEditor(tk.Toplevel):
    def __init__(self, app: WindowsMonitorApp, target: Optional[WinHost] = None, on_save=None):
        ...
        self.title("Add / Edit Windows Host")
        ...
        row += 1
        ttk.Label(body, text="Host / ComputerName:").grid(row=row, column=0, sticky="e", padx=4, pady=4)
        ...
        row += 1
        self.cred_box = ttk.LabelFrame(body, text="Credential")
        self.cred_box.grid(row=row, column=0, columnspan=4, sticky="ew", padx=2, pady=6)
    ...
    def _refresh(self):
        ...
        for child in self.cred_box.winfo_children():
            child.configure(state=("normal" if use_cred else "disabled"))
    ...
    def _test_connection(self):
        try:
            ...
            if data:
                messagebox.showinfo(APP_NAME, f"Connection OK: {h.name}")
            else:
    ...
    def _save(self):
        try:
            ...
            if not t.name:
                ...
                return
            if not t.host:
                messagebox.showerror(APP_NAME, "Host/ComputerName is required.")
                return
            if self.on_save:
                self.on_save(t)
            self.destroy()
...
class WindowsPlaceholder(WindowsMonitorApp):
    pass


# ===== Safe handler patch (Windows) =====
def _win__on_button3(self, event):
    try:
...
def _win__select_columns_dialog(self):
    top = tk.Toplevel(self)
    top.title("Customize Columns — Windows")
...
def _win__open_filter_dialog(self):
    top = tk.Toplevel(self)
    top.title("Filter — Windows")
    top.resizable(False, False)
    frm = ttk.Frame(top, padding=10)
    frm.pack(fill=tk.BOTH, expand=True)
...
# Bind patches
try:
    WindowsMonitorApp._on_button3 = _win__on_button3
    WindowsMonitorApp._select_columns_dialog = _win__select_columns_dialog
    WindowsMonitorApp._select_column_doalog = _win__select_columns_dialog  # compatibility alias
    WindowsMonitorApp._open_filter_dialog = _win__open_filter_dialog
    WindowsMonitorApp._clear_filter = _win__clear_filter
