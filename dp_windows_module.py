...
import base64, json, os, re, subprocess, sys, threading, time, tkinter as tk
...
def load_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "common_login": {"user":"", "password_enc":""},
        "email": {"server":"", "port":25, "from_addr":"", "to_addrs":"", "subject":"Windows Health Report"},
        "column_order": [],
        "visible_columns": [],
        "email_columns": [],
        "header_filters": {},
...
function Get-OSData([string]$CN,[System.Management.Automation.PSCredential]$cred) {
  try {
    ...
    if ($sess) {
      $os = Get-CimInstance -CimSession $sess Win32_OperatingSystem
      $cpu = Get-CimInstance -CimSession $sess Win32_Processor | Select-Object -First 1
      $disks = Get-CimInstance -CimSession $sess Win32_LogicalDisk -Filter "DriveType=3"
      $hf = Get-CimInstance -CimSession $sess Win32_QuickFixEngineering | Sort-Object InstalledOn -Descending | Select-Object -First 1
    } else {
      $os = Get-WmiObject -Class Win32_OperatingSystem -ComputerName $CN -Credential $cred
      $cpu = Get-WmiObject -Class Win32_Processor -ComputerName $CN -Credential $cred | Select-Object -First 1
    ...
    foreach ($d in $disks) {
      if ($d.Size -gt 0) {
        $used = 100.0 * (1.0 - ($d.FreeSpace / $d.Size))
        if ($used -ge 90.0) { $cnt90 += 1 }
      }
    }
    ...
    $out = [ordered]@{
      ...
      os_edition = $edition
      memory_gb = $memGB
      cpu_count = [int]$cpu.NumberOfLogicalProcessors
      mem_used_pct = $usedPct
      cpu_load_pct = [Math]::Round([double]$cpuLoad,1)
      disks90 = $cnt90
      uptime_sec = $uptime
      security_patch = $kbs
      host = $CN
...
class WindowsMonitorApp(tk.Frame):
    LOGICAL_COLUMNS = (
        ...
        "Security Patch","Memory (GB)","CPU Count","Memory Usage %","CPU Load Avg %",
        "Disks >90%","Uptime","Last Checked","Check Status","Error"
    )
    ...
    def _build_ui(self):
        ...
        for col in self.LOGICAL_COLUMNS:
            ...
            self.tree.column(col,width=150,stretch=True,anchor="w")
    ...
    def _build_html(self, rows: List[List]) -> str:
        ...
        def cell_style(text: str, col: str) -> str:
            ...
            if col=="CPU Load Avg %":
                ...
                return "background-color:#ffe6e6;color:#7a0000;font-weight:bold;" if v>=80.0 else ""
    ...
    def _auth_settings(self):
        ...
        cmn=self.cfg.get("common_login",{"user":"","password_enc":""})
        ttk.Label(dlg,text="Username (DOMAIN\\user or user@domain):").grid(row=1,column=0,sticky="e",**pad); user_var=tk.StringVar(value=cmn.get("user","")); ttk.Entry(dlg,textvariable=user_var,width=34).grid(row=1,column=1,sticky="w",**pad)
        ttk.Label(dlg,text="Password:").grid(row=2,column=0,sticky="e",**pad); pw_var=tk.StringVar(value=""); ttk.Entry(dlg,textvariable=pw_var,show="*",width=34).grid(row=2,column=1,sticky="w",**pad)
        def save_now():
            self.cfg["use_common_login"]=use_var.get()
            enc=_win_protect(pw_var.get().encode()) if pw_var.get() else cmn.get("password_enc","")
            self.cfg["common_login"]={"user":user_var.get().strip(),"password_enc":enc}
...
class HostEditor(tk.Toplevel):
    def __init__(self, app: WindowsMonitorApp, target: Optional[Host] = None, on_save=None):
        ...
        self.title("Add / Edit Windows Server"); self.resizable(False,False)
        pad={"padx":8,"pady":4}
        ...
        ttk.Label(self,text="Username (DOMAIN\\user or user@domain):").grid(row=4,column=0,sticky="e",**pad); ttk.Entry(self,textvariable=user,width=28).grid(row=4,column=1,sticky="w",**pad)
        ttk.Label(self,text="Password:").grid(row=5,column=0,sticky="e",**pad); ttk.Entry(self,textvariable=pw,show="*",width=28).grid(row=5,column=1,sticky="w",**pad)

        btns=ttk.Frame(self); btns.grid(row=6,column=0,columnspan=2,sticky="ew",**pad)
        def save():
            enc=_win_protect(pw.get().encode()) if pw.get() else (self.target.password_enc if self.target else "")
            h=Host(name=name.get().strip(), environment=env.get().strip(), host=host.get().strip(), user=user.get().strip(), password_enc=enc)
