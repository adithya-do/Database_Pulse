...
# -*- coding: utf-8 -*-
"""
Database Pulse — App Launcher (v1.2)
Adds Linux/Unix Servers and Windows Servers modules.
"""
import sys
...
def _lazy_import_linux():
    import dp_linux_module as mod
    return mod

def _lazy_import_windows():
    import dp_windows_module as mod
    return mod
...
class Landing(ttk.Frame):
    def __init__(self, master, version_text: str):
        ...
        icon = tk.Label(center, text="🗄", font=("Segoe UI Emoji", 48))
        icon.pack(pady=(0, 8))
        ...
        subtitle = tk.Label(center, text="Unified Health Checks for Databases & Servers", font=("Segoe UI", 9))
...
class RouterApp(tk.Tk):
    def __init__(self):
        ...
        mkbtn("Linux/Unix Servers", "linux", self.show_linux)
        mkbtn("Windows Servers", "windows", self.show_windows)
        ...
        self.stack.grid_rowconfigure(0, weight=1); self.stack.grid_columnconfigure(0, weight=1)

        # Views/containers
        self.view_landing = Landing(self.stack, APP_VERSION)

        self.oracle_container = tk.Frame(self.stack); self.oracle_container.grid_rowconfigure(0, weight=1); self.oracle_container.grid_columnconfigure(0, weight=1)
        self.sql_container    = tk.Frame(self.stack); self.sql_container.grid_rowconfigure(0, weight=1); self.sql_container.grid_columnconfigure(0, weight=1)
        self.linux_container  = tk.Frame(self.stack); self.linux_container.grid_rowconfigure(0, weight=1); self.linux_container.grid_columnconfigure(0, weight=1)
        self.win_container    = tk.Frame(self.stack); self.win_container.grid_rowconfigure(0, weight=1); self.win_container.grid_columnconfigure(0, weight=1)

        self.view_oracle: Optional[tk.Frame] = None
        self.view_sql: Optional[tk.Frame] = None
        self.view_linux: Optional[tk.Frame] = None
        self.view_windows: Optional[tk.Frame] = None
    ...
    def _select_nav(self, key: str, route_call):
        ...
        if key in self._nav_btns:
            self._nav_btns[key].configure(bg="#6ea8fe")
        route_call()

    def _show(self, frame: tk.Frame):
        for child in list(self.stack.winfo_children()):
            for f in (child.grid_forget, child.pack_forget, child.place_forget):
                try: f()
                except Exception: pass
        try: frame.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
        try: frame.tkraise()
        except Exception: pass

    def show_home(self):
        try: self.view_landing.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
    ...
    def show_oracle(self):
        if self.view_oracle is None:
            ...
            if not any(getattr(w, "__class__", None).__name__ == "MonitorApp" for w in self.oracle_container.winfo_children()):
                mon = mod.MonitorApp(self.oracle_container, mod.load_config())
                try: mon.grid(row=0, column=0, sticky="nsew")
                except Exception: pass
        try: self.view_oracle.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
    ...
    def show_sqlserver(self):
        ...
        try: self.view_sql.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
        self._show(self.view_sql)

    def show_linux(self):
        if self.view_linux is None:
            mod = _lazy_import_linux()
            self.view_linux = self.linux_container
            if not self.linux_container.winfo_children():
                inner = mod.LinuxPlaceholder(self.linux_container)
                inner.grid(row=0, column=0, sticky="nsew")
        try: self.view_linux.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
        self._show(self.view_linux)

    def show_windows(self):
        if self.view_windows is None:
            mod = _lazy_import_windows()
            self.view_windows = self.win_container
            if not self.win_container.winfo_children():
                inner = mod.WindowsPlaceholder(self.win_container)
                inner.grid(row=0, column=0, sticky="nsew")
        try: self.view_windows.grid(row=0, column=0, sticky="nsew")
        except Exception: pass
        self._show(self.view_windows)
