#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# SQL Server module for Database Pulse — with Excel-style header filters
# and backup threshold. Includes all missing handlers that the main app calls.

from __future__ import annotations

import base64, json, locale, subprocess, sys, threading, time
...
class InstanceTarget:
    ...
    win_user_display: Optional[str] = None
...
class InstanceHealth:
    ...
    db_total: int = 0
    db_online: int = 0
    oldest_full_backup: Optional[str] = None
    elapsed_ms: int = 0
    error: str = ""
    ts: str = field(default_factory=lambda: datetime.now().strftime("%Y-%m-%d %H:%M:%S"))

def logical_columns() -> List[str]:
    return ["S.No","SQL Server Instance","Environment","Version","Edition","CU","Instance Status","Agent Status","DB Status","Last Full Backup","Last Checked","Check Status","Error"]
...
def default_config() -> Dict[str, Any]:
    ...
    return {
        ...
        "instances": [],
        "sqlcmd_path": "",
        "email": {"server":"", "port":25, "from_addr":"", "to_addrs":"", "subject":"SQL Server Health Report"},
        "last_health": {},
        "auto_run": False,
        "column_order": cols[:],
...
def _serialize_inst(i: InstanceTarget) -> Dict[str, Any]:
    return {"name":i.name,"server":i.server,"environment":i.environment,"auth":i.auth,"username":i.username,"password_enc":i.password_enc,"win_user_display":i.win_user_display}

def _hydrate_inst(d: Dict[str, Any]) -> InstanceTarget:
    return InstanceTarget(name=d.get("name",""), server=d.get("server",""), environment=d.get("environment","NON-PROD"), auth=d.get("auth","windows"), username=d.get("username"), password_enc=d.get("password_enc"), win_user_display=d.get("win_user_display"))
    ...
    if CONFIG_PATH.exists():
        try:
            ...
            for k, v in {"last_health":{}, "column_order":default_config()["column_order"], "visible_columns":default_config()["visible_columns"], "email_columns":default_config()["email_columns"], "column_widths":{}, "active_filter":[], "header_filters":{}}.items():
                base.setdefault(k, v)
            if base.get("instances"): base["instances"]=[_serialize_inst(_hydrate_inst(i)) for i in base["instances"]]
            return base
...
def save_config(cfg: Dict[str, Any]):
    out=dict(cfg)
    out["instances"]=[_serialize_inst(_hydrate_inst(i) if isinstance(i,dict) else i) for i in cfg.get("instances",[])]
    with open(CONFIG_PATH,"w",encoding="utf-8") as f: json.dump(out,f,indent=2,default=str)

def build_sqlcmd_command(sqlcmd_path: str, server: str, auth: str, username: Optional[str], password: Optional[str], query: str, login_timeout: int = 10, query_timeout: int = 30) -> List[str]:
    exe = sqlcmd_path.strip() or "sqlcmd"
    if not server: raise ValueError("Server/Instance is empty. Please set 'Server\\Instance' or 'host,port'.")
    cmd=[exe,"-S",server]
    if auth=="windows": cmd+=["-E"]
    else:
        if not username or password is None: raise ValueError("SQL authentication requires username and password.")
...
def run_sqlcmd(cmd: List[str]) -> Tuple[int,str,str]:
    ...
    try:
        ...
        return p.returncode, out_b.decode(enc,errors="ignore").strip(), err_b.decode(enc,errors="ignore").strip()
...
def q_version_edition_cu() -> str:
    return ("SET NOCOUNT ON; SELECT CONVERT(varchar(10), SERVERPROPERTY('ProductMajorVersion')) AS major, "
...
def q_services() -> str:
    return "SET NOCOUNT ON; SELECT servicename, status_desc FROM sys.dm_server_services;"

def q_db_status() -> str:
    return ("SET NOCOUNT ON; SELECT COUNT(*) AS total, SUM(CASE WHEN state_desc='ONLINE' THEN 1 ELSE 0 END) AS online "
            "FROM sys.databases WHERE database_id > 0;")

def q_oldest_full_backup() -> str:
    return ("SET NOCOUNT ON; WITH last_full AS ( SELECT d.name, MAX(b.backup_finish_date) AS last_full "
            "FROM sys.databases d LEFT JOIN msdb.dbo.backupset b ON b.database_name = d.name AND b.type = 'D' "
            "WHERE d.database_id > 0 GROUP BY d.name ) SELECT CONVERT(varchar(19), MIN(last_full), 120) AS oldest_full FROM last_full;")

def map_major_to_year(major: str) -> str:
    try: m=int(major)
    except Exception: return major
    return {16:"2022",15:"2019",14:"2017",13:"2016",12:"2014",11:"2012"}.get(m,str(m))

class SqlServerMonitorApp(ttk.Frame):
    LOGICAL_COLUMNS = tuple(logical_columns())
    STATUS_COLUMNS = {"Version","Edition","CU","Instance Status","Agent Status","DB Status","Last Full Backup","Last Checked","Check Status","Error"}

    def __init__(self, master):
        super().__init__(master)
        self.cfg=load_config()
        self.interval_sec=int(self.cfg.get("interval_sec",DEFAULT_INTERVAL_SEC))
        self.instances=[_hydrate_inst(i) if isinstance(i,dict) else i for i in self.cfg.get("instances",[])]
    ...
    def _build_ui(self):
        ...
        style.configure("SS.Treeview.Heading", background="#cfe8ff", foreground="#000", font=(self._font.actual("family"), self._font.actual("size"), "bold"))
    ...
    def _row_passes_header(self, values: List[Any]) -> bool:
        ...
        for col,selected in self._header_filters.items():
            ...
            if i is None or i>=len(values): return False
            val=str(values[i])
            if isinstance(selected,set) and len(selected)==0: return False
            if isinstance(selected,set) and val not in selected: return False
        return True
    ...
    def _loop(self):
        ...
        self._checks_async(self.instances); self.after(self.interval_var.get()*1000,self._loop)

    def _pick_sqlcmd(self):
        p=filedialog.askopenfilename(title="Locate sqlcmd.exe",filetypes=[("sqlcmd","sqlcmd.exe"),("All","*.*")])
        if p:
            self.sqlcmd_path_var.set(p); self.cfg["sqlcmd_path"]=p; save_config(self.cfg)
    ...
    def _parse_datecell(self,s:str)->float:
        ...
        try:
            ...
            dt=datetime.strptime(parts,"%Y-%m-%d %H:%M:%S"); return dt.timestamp()
        except Exception:
            try: dt=datetime.strptime(t.split()[-1],"%Y-%m-%d"); return dt.timestamp()
    ...
    def _generic_key(self,col:str,s:str):
        ...
        if col in ("Instance Status",): return (self._status_rank(s), s)
        if col=="DB Status":
            a,b=self._parse_db_status(s); return (a/b if b else -1.0,a,b)
        if col in ("Last Full Backup","Last Checked"): return (self._parse_datecell(s),)
        return (str(s).lower(),)
    ...
    def _apply_persisted_row(self,name:str,h:Dict[str,Any]):
        ...
        db_total=int(h.get("db_total",0) or 0); db_online=int(h.get("db_online",0) or 0); db_ok=(db_total==db_online) and db_total>0
        last_full=h.get("oldest_full_backup","-"); ok_backup=False
        if last_full and last_full!="-":
            try: dt=datetime.strptime(last_full,"%Y-%m-%d %H:%M:%S"); ok_backup=(datetime.now()-dt)<=timedelta(days=2)
            except Exception: ok_backup=False
        vals_by={c:i for i,c in enumerate(self.LOGICAL_COLUMNS)}
        vals[vals_by["Version"]]=h.get("version_year","-"); vals[vals_by["Edition"]]=h.get("edition","-"); vals[vals_by["CU"]]=h.get("cu","-")
        ...
        vals[vals_by["Last Checked"]]=h.get("ts","-"); vals[vals_by["Check Status"]]="Complete"; vals[vals_by["Error"]]=h.get("error","")
    ...
    def _export_json(self):
        ...
        try:
            export={
                ...
                "email": {"server": self.smtp_server_var.get().strip(), "port": int(self.smtp_port_var.get() or 25), "from_addr": self.from_var.get().strip(), "to_addrs": self.to_var.get().strip(), "subject": self.cfg.get("email",{}).get("subject","SQL Server Health Report")},
    ...
    # ---------- Run checks ----------
    def run_all_once(self): self._checks_async(self.instances)

    def run_selected_once(self):
        sel=self.tree.selection()
        if not sel: messagebox.showinfo(APP_NAME,"Select a row (Instance) to run."); return
        name=sel[0]; inst=next((x for x in self.instances if x.name==name),None)
        if not inst: messagebox.showerror(APP_NAME,"Selected instance not found."); return
    ...
    def _checks_async(self,instances:List[InstanceTarget]):
        for t in instances: self._set_check_status(t.name,"In Progress")
        def job(inst:InstanceTarget):
            res=self._check_one(inst); self.after(0,lambda n=inst.name,i=inst,h=res: self._apply_result(n,i,h))
        for inst in instances: threading.Thread(target=job,args=(inst,),daemon=True).start()

    def _check_one(self,inst:InstanceTarget)->InstanceHealth:
        t0=time.time(); h=InstanceHealth(); sqlcmd_path=self.sqlcmd_path_var.get().strip() or self.cfg.get("sqlcmd_path","")
        try:
            cmd=build_sqlcmd_command(sqlcmd_path,inst.server,inst.auth,inst.username,_decrypt_password(inst.password_enc), q_version_edition_cu())
            rc,out,err=run_sqlcmd(cmd)
            if rc==0:
                rows=parse_scalar_list(out)
                if rows:
                    major=rows[0][0]; edition=rows[0][1] if len(rows[0])>1 else "-"; cu=rows[0][2] if len(rows[0])>2 else "-"
                    h.version_year=map_major_to_year(major); h.edition=edition or "-"; h.cu=cu or "-"
            else: raise RuntimeError(err or "version/edition/CU query failed")

            cmd=build_sqlcmd_command(sqlcmd_path,inst.server,inst.auth,inst.username,_decrypt_password(inst.password_enc), q_services())
            rc,out,err=run_sqlcmd(cmd)
            if rc==0:
                rows=parse_scalar_list(out)
                for r in rows:
                    if len(r)<2: continue
                    svc,status=r[0],r[1]; su=svc.upper()
                    if "SQL SERVER AGENT" in su: h.agent_status=status
                    elif "SQL SERVER" in su: h.instance_status=status

            cmd=build_sqlcmd_command(sqlcmd_path,inst.server,inst.auth,inst.username,_decrypt_password(inst.password_enc), q_db_status())
            rc,out,err=run_sqlcmd(cmd)
            if rc==0:
                rows=parse_scalar_list(out)
                if rows and len(rows[0])>=2:
                    h.db_total=int(rows[0][0] or 0); h.db_online=int(rows[0][1] or 0)

            cmd=build_sqlcmd_command(sqlcmd_path,inst.server,inst.auth,inst.username,_decrypt_password(inst.password_enc), q_oldest_full_backup())
            rc,out,err=run_sqlcmd(cmd)
            if rc==0:
                rows=parse_scalar_list(out)
                if rows and rows[0] and rows[0][0]: h.oldest_full_backup=rows[0][0]

            h.elapsed_ms=int((time.time()-t0)*1000); return h
        except Exception as e:
            h.error=str(e); h.elapsed_ms=int((time.time()-t0)*1000); return h

    def _apply_result(self,name:str,inst:InstanceTarget,h:InstanceHealth):
        def mark(ok:bool)->str: return GOOD if ok else BAD
        inst_ok=h.instance_status.upper().startswith("RUN"); agent_ok=h.agent_status.upper().startswith("RUN")
        db_ok=(h.db_total==h.db_online) and h.db_total>0
        last_full_txt=h.oldest_full_backup or "-"; ok_backup=False
        if h.oldest_full_backup:
            try: dt=datetime.strptime(h.oldest_full_backup,"%Y-%m-%d %H:%M:%S"); ok_backup=(datetime.now()-dt)<=timedelta(days=2)
            except Exception: ok_backup=False
        vals=list(self.tree.item(name)["values"] or ["-"]*len(self.LOGICAL_COLUMNS))
        colidx={c:i for i,c in enumerate(self.LOGICAL_COLUMNS)}
        vals[colidx["Version"]]=h.version_year or "-"; vals[colidx["Edition"]]=h.edition or "-"; vals[colidx["CU"]]=h.cu or "-"
        ...
        self.tree.item(name,values=vals)
        self.last_health[name]={"version_year":h.version_year,"edition":h.edition,"cu":h.cu,"instance_status":h.instance_status,"agent_status":h.agent_status,"db_total":h.db_total,"db_online":h.db_online,"oldest_full_backup":h.oldest_full_backup or "-","ts":h.ts,"error":h.error or ""}
    ...
    def _clear_all_rows(self):
        ...
        self.status_var.set("Cleared all rows (except S.No, SQL Server Instance, Environment)."); self._apply_all_filters()
    ...
    # ---------- CRUD dialogs ----------
    def _add_dialog(self): InstanceEditor(self,on_save=self._add_instance)
    def _edit_selected(self):
        sel=self.tree.selection()
        if not sel: messagebox.showinfo(APP_NAME,"Select a row to edit."); return
        name=sel[0]; t=next((x for x in self.instances if x.name==name),None)
        if not t: messagebox.showerror(APP_NAME,"Instance not found."); return
        InstanceEditor(self,target=t,on_save=self._update_instance)
    def _remove_selected(self):
        sel=self.tree.selection()
        if not sel: return
        name=sel[0]; self.instances=[i for i in self.instances if i.name!=name]
        self._detached.discard(name); self.tree.delete(name); self._persist_instances(); self._renumber()
    def _add_instance(self,i:InstanceTarget):
        if any(x.name==i.name for x in self.instances): messagebox.showerror(APP_NAME,"An instance with this name already exists."); return
        self.instances.append(i); self._persist_instances()
        values=["-"]*len(self.LOGICAL_COLUMNS); values[0]=len(self.instances); values[1]=i.name; values[2]=i.environment
        self.tree.insert("",tk.END,iid=i.name,values=tuple(values)); self._renumber(); self._autosize_columns(); self._apply_all_filters()
    def _update_instance(self,i:InstanceTarget):
        found=False
        for idx,cur in enumerate(self.instances):
            if cur.name==i.name: self.instances[idx]=i; found=True; break
        if not found: self.instances.append(i)
        self._persist_instances()
        if i.name in self.tree.get_children("") or i.name in self._detached:
            if i.name in self._detached: self.tree.move(i.name,"","end"); self._detached.discard(i.name)
            vals=list(self.tree.item(i.name)["values"]); vals[1]=i.name; vals[2]=i.environment; self.tree.item(i.name,values=vals); self._apply_all_filters()
        self._autosize_columns()
...
class InstanceEditor(tk.Toplevel):
    def __init__(self, app: SqlServerMonitorApp, target: Optional[InstanceTarget] = None, on_save=None):
        ...
        self.title("Add / Edit SQL Server Instance"); self.resizable(False,False)
        ...
        row=0; ttk.Label(body,text="Instance Name (display):").grid(row=row,column=0,sticky="e",padx=4,pady=4)
        ...
        row+=1; ttk.Label(body,text="Server\\Instance or host,port:").grid(row=row,column=0,sticky="e",padx=4,pady=4)
        ...
        row+=1; self.win_box=ttk.LabelFrame(body,text="Windows Authentication"); self.win_box.grid(row=row,column=0,columnspan=4,sticky="ew",padx=2,pady=6)
        ...
        row+=1; self.sql_box=ttk.LabelFrame(body,text="SQL Server Authentication"); self.sql_box.grid(row=row,column=0,columnspan=4,sticky="ew",padx=2,pady=6)
    ...
    def _refresh(self):
        ...
        for child in self.sql_box.winfo_children(): child.configure(state=("normal" if sql_auth else "disabled"))
        for child in self.win_box.winfo_children(): child.configure(state=("normal" if not sql_auth else "disabled"))

    def _make_target(self)->InstanceTarget:
        name=self.name_var.get().strip(); env=self.env_var.get().strip() or "NON-PROD"
        server=self.server_var.get().strip(); auth=self.auth_var.get()
        if auth=="sql":
            user=self.user_var.get().strip(); pwd=self.pass_var.get()
            return InstanceTarget(name=name,server=server,environment=env,auth="sql",username=user or None,password_enc=_encrypt_password(pwd) if pwd else None,win_user_display=None)
        else:
            return InstanceTarget(name=name,server=server,environment=env,auth="windows",username=None,password_enc=None,win_user_display=self.win_user_display.get().strip() or None)

    def _test_connection(self):
        try:
            t=self._make_target(); sqlcmd_path=self.app.sqlcmd_path_var.get().strip() or self.app.cfg.get("sqlcmd_path","")
            cmd=build_sqlcmd_command(sqlcmd_path,t.server,t.auth,t.username,_decrypt_password(t.password_enc),"SET NOCOUNT ON; SELECT 1;")
            rc,out,err=run_sqlcmd(cmd)
            if rc==0 and out.strip().startswith("1"): messagebox.showinfo(APP_NAME,f"Connection OK: {t.name}")
            else: raise RuntimeError(err or out or "Unknown error")
        except Exception as e: messagebox.showerror(APP_NAME,f"Connection failed:\n{e}")

    def _save(self):
        try:
            t=self._make_target()
            if not t.name: messagebox.showerror(APP_NAME,"Instance Name is required."); return
            if not t.server: messagebox.showerror(APP_NAME,"Server/Instance is required."); return
            if self.on_save: self.on_save(t)
            self.destroy()
        except Exception as e: messagebox.showerror(APP_NAME,f"Failed to save: {e}")

# Some apps import this name; keep it for compatibility
class SqlServerPlaceholder(SqlServerMonitorApp): pass
