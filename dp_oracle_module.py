...
SQLS = {
    ...
    "bk_data":("select max(END_TIME) from V$RMAN_BACKUP_JOB_DETAILS "
               "where INPUT_TYPE in ('DB INCR','DB FULL') and STATUS='COMPLETED'"),
    "bk_arch":("select max(END_TIME) from V$RMAN_BACKUP_JOB_DETAILS "
               "where INPUT_TYPE in ('ARCHIVELOG') and STATUS='COMPLETED'"),
}
